...
export class CheckInUseCaseCase {
  ...
  async execute({
    ...
    const MAX_DISTANCE_IN_KILOMETERS = 0.1 // constants for the max distance (best practice)

    if (distance > MAX_DISTANCE_IN_KILOMETERS) {
      throw new MaxDistanceError()
    }

    const checkInSameDay = await this.checkInsRepository.findByIdUserIdOnDate(
      userId,
      new Date(),
    )
